let onebit =
    0; // set this bit to zero, to keep multiple accordion items open at a time
let accordionItems = document.body.querySelectorAll('.accordion-items');
accordionItems.forEach(
    accordionItem => {accordionItem.addEventListener('click', event => {
      ...
      if (accordionItem.firstElementChild.firstElementChild.classList.contains(
              'hidden')) {
